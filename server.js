...
const transporter = nodemailer.createTransport({
  ...
  secure: process.env.SMTP_SECURE === "true",
  auth: {
    user: process.env.EMAIL_USER, // Use EMAIL_USER for both
    pass: process.env.EMAIL_PASSWORD, // Use EMAIL_PASSWORD for both
  },
});
...
const imapConfig = {
  imap: {
    user: process.env.EMAIL_USER, // Use EMAIL_USER for both
    password: process.env.EMAIL_PASSWORD, // Use EMAIL_PASSWORD for both
    host: process.env.IMAP_HOST || "imap.gmail.com",
    port: parseInt(process.env.IMAP_PORT) || 993,
    tls: true,
    tlsOptions: { rejectUnauthorized: false }
...
app.listen(PORT, () => {
  console.log(`🚀 Server running on http://localhost:${PORT}`);
  console.log(`📧 IMAP User: ${process.env.EMAIL_USER}`);
