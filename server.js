...
app.get('/debug-oauth', (req, res) => {
  const debugInfo = {
    ...
    smtp: {
      ...
      password: EMAIL_PASSWORD ? 'Set (length: ' + EMAIL_PASSWORD.length + ')' : 'MISSING'
    },
    timestamp: new Date().toISOString()
  };
...
app.get('/env-check', (req, res) => {
  const envVars = {
    EMAIL_USER: EMAIL_USER ? 'Set' : 'Missing',
    EMAIL_PASSWORD: EMAIL_PASSWORD ? `Set (length: ${EMAIL_PASSWORD.length})` : 'Missing',
    MONGODB_URI: process.env.MONGODB_URI ? 'Set' : 'Missing',
...
// ✅ FIXED: Check authorization BEFORE sending WebSocket
if (isPositive) {
  const isAuthorized = await checkIfUserAuthorized(trimmedUserId);
  
  if (isAuthorized) {
    // User is already authorized - send "proceed" WebSocket nudge
    const payload = {
      type: 'proceed',
      userId: trimmedUserId,
      jobId: trimmedJobId,
    ...
    console.log('🚀 Sending WebSocket nudge (authorized user - frontend will check DB)');
...
app.get('/auth/use-company-email', async (req, res) => {
  ...
  try {
    ...
    const companyEmail = process.env.COMPANY_EMAIL || EMAIL_USER;
